...
/**
 * DETAILED AJAX DEBUGGING - Add this to your theme's functions.php
 * This will help us see exactly what's happening with the AJAX requests
 */

// Add detailed logging to all AJAX requests
add_action('wp_ajax_wpetp_track_event', 'wpetp_debug_ajax_request', 1);
add_action('wp_ajax_nopriv_wpetp_track_event', 'wpetp_debug_ajax_request', 1);

function wpetp_debug_ajax_request() {
    error_log('=== WPETP AJAX DEBUG ===');
    error_log('Action: ' . ($_POST['action'] ?? 'none'));
    error_log('POST data: ' . print_r($_POST, true));
    error_log('Nonce provided: ' . ($_POST['nonce'] ?? 'none'));
    error_log('User logged in: ' . (is_user_logged_in() ? 'yes' : 'no'));
    error_log('Current user ID: ' . get_current_user_id());
    
    // Test nonce verification
    if (isset($_POST['nonce'])) {
        $nonce_check = wp_verify_nonce($_POST['nonce'], 'wpetp_frontend_nonce');
        error_log('Nonce verification result: ' . ($nonce_check ? 'PASS' : 'FAIL'));
        
        // Try different nonce actions
        $alt_checks = array(
            'wpetp_frontend_ajax' => wp_verify_nonce($_POST['nonce'], 'wpetp_frontend_ajax'),
            'wpetp_admin_nonce' => wp_verify_nonce($_POST['nonce'], 'wpetp_admin_nonce'),
            'wpetp_nonce' => wp_verify_nonce($_POST['nonce'], 'wpetp_nonce'),
        );
        error_log('Alternative nonce checks: ' . print_r($alt_checks, true));
    }
    
    error_log('=== END AJAX DEBUG ===');
}

// Add a simple test AJAX handler to verify AJAX is working at all
add_action('wp_ajax_wpetp_simple_test', 'wpetp_simple_test_handler');
add_action('wp_ajax_nopriv_wpetp_simple_test', 'wpetp_simple_test_handler');

function wpetp_simple_test_handler() {
    error_log('Simple test AJAX called - this proves AJAX is working');
    wp_send_json_success('Simple test successful');
}

// Enhanced frontend debug with detailed AJAX testing
add_action('wp_footer', 'wpetp_enhanced_debug_script');

function wpetp_enhanced_debug_script() {
    if (!is_front_page()) return;
    ?>
    <script>
    console.log('=== ENHANCED WPETP DEBUG ===');
    
    // Check all our data
    console.log('wpetp_data:', typeof wpetp_data !== 'undefined' ? wpetp_data : 'UNDEFINED');
    console.log('wpetpConfig:', typeof wpetpConfig !== 'undefined' ? wpetpConfig : 'UNDEFINED');
    
    // Test 1: Simple AJAX test (no nonce required)
    jQuery.ajax({
        url: '<?php echo admin_url('admin-ajax.php'); ?>',
        type: 'POST',
        data: {
            action: 'wpetp_simple_test'
        },
        success: function(response) {
            console.log('✓ Simple AJAX test passed:', response);
        },
        error: function(xhr, status, error) {
            console.error('✗ Simple AJAX test failed:', {
                status: xhr.status,
    ...
    // Test 2: Check if we have the right nonce
    if (typeof wpetp_data !== 'undefined' && wpetp_data.nonce) {
        console.log('Testing WPETP nonce:', wpetp_data.nonce);
        
        // Test the actual tracking call with minimal data
        jQuery.ajax({
            url: wpetp_data.ajax_url,
            type: 'POST',
            data: {
                action: 'wpetp_track_event',
                event_id: 999, // Fake event ID for testing
                event_data: {test: true},
                nonce: wpetp_data.nonce
            },
            success: function(response) {
                console.log('✓ WPETP AJAX test passed:', response);
            },
            error: function(xhr, status, error) {
                console.error('✗ WPETP AJAX test failed:', {
                    status: xhr.status,
                    statusText: xhr.statusText,
                    responseText: xhr.responseText,
                    error: error,
                ...
                // Show the actual response body if available
                if (xhr.responseText) {
        ...
        console.error('✗ wpetp_data.nonce not available');
    }
    
    // Test 3: Check what events we have
    if (typeof wpetp_data !== 'undefined' && wpetp_data.events) {
        console.log('Available events:', wpetp_data.events);
        
        // Look for page view events specifically
        const pageViewEvents = wpetp_data.events.filter(e => e.trigger_type === 'page_view');
        console.log('Page view events:', pageViewEvents);
        
        if (pageViewEvents.length > 0) {
            console.log('Found page view events - they should trigger automatically');
        } else {
            console.warn('No page view events found - you need to create one');
        }
    }
...
// Debug: Show what AJAX actions are actually registered
add_action('admin_notices', 'wpetp_show_registered_actions');

function wpetp_show_registered_actions() {
    ...
    if (isset($wp_filter['wp_ajax_wpetp_track_event'])) {
        $ajax_actions[] = 'wp_ajax_wpetp_track_event (logged in users)';
    }
    if (isset($wp_filter['wp_ajax_nopriv_wpetp_track_event'])) {
        $ajax_actions[] = 'wp_ajax_nopriv_wpetp_track_event (non-logged users)';
    }
    if (isset($wp_filter['wp_ajax_wpetp_simple_test'])) {
        $ajax_actions[] = 'wp_ajax_wpetp_simple_test (test action)';
    }
    
    if (!empty($ajax_actions)) {
        echo '<div class="notice notice-success">';
        echo '<h4>WPETP AJAX Actions Registered:</h4>';
        echo '<ul>';
        ...
        echo '<h4>⚠️ No WPETP AJAX Actions Found!</h4>';
        echo '<p>The WPETP Frontend class may not be loading properly.</p>';
        echo '</div>';
    }
}

/**
 * Add this to your theme's functions.php to debug AJAX registration
 */

// Check if Frontend class exists and is being instantiated
add_action('init', 'wpetp_debug_frontend_class', 999);

function wpetp_debug_frontend_class() {
    error_log('=== WPETP Frontend Class Debug ===');
    
    // Check if class exists
    if (class_exists('WPETP_Frontend')) {
        error_log('✓ WPETP_Frontend class exists');
        
        // Try to get instance
        try {
            $frontend = WPETP_Frontend::get_instance();
            if ($frontend) {
                error_log('✓ WPETP_Frontend instance created successfully');
            } else {
                error_log('✗ WPETP_Frontend instance is null');
            }
        } catch (Exception $e) {
            error_log('✗ Error getting WPETP_Frontend instance: ' . $e->getMessage());
        }
    } else {
        error_log('✗ WPETP_Frontend class does not exist');
    }
    
    // Check what AJAX actions are registered
    global $wp_filter;
    
    $ajax_hooks_to_check = array(
        'wp_ajax_wpetp_track_event',
        'wp_ajax_nopriv_wpetp_track_event',
        'wp_ajax_wpetp_frontend_track',
        'wp_ajax_nopriv_wpetp_frontend_track'
    );
    
    foreach ($ajax_hooks_to_check as $hook) {
        if (isset($wp_filter[$hook])) {
            error_log("✓ Hook registered: $hook");
            
            // Show what callbacks are attached
            if (isset($wp_filter[$hook]->callbacks)) {
                foreach ($wp_filter[$hook]->callbacks as $priority => $callbacks) {
                    foreach ($callbacks as $callback) {
                        ...
                        error_log("  - Callback: $callback_info (priority: $priority)");
                    }
                }
            }
        } else {
            error_log("✗ Hook NOT registered: $hook");
        }
    }
    
    error_log('=== End Frontend Class Debug ===');
}

// Add a direct AJAX handler to test if AJAX works at all
add_action('wp_ajax_wpetp_direct_test', 'wpetp_direct_test_handler');
add_action('wp_ajax_nopriv_wpetp_direct_test', 'wpetp_direct_test_handler');

function wpetp_direct_test_handler() {
    error_log('Direct AJAX test handler called successfully');
    wp_send_json_success('Direct AJAX test works');
}

// Check if the main plugin file is loading the Frontend class
add_action('wp_loaded', 'wpetp_debug_plugin_loading', 999);

function wpetp_debug_plugin_loading() {
    error_log('=== WPETP Plugin Loading Debug ===');
    
    // Check if main plugin class exists
    if (class_exists('WP_Event_Tracker_Pro')) {
        error_log('✓ Main plugin class exists');
        
        // Check if Performance class is loading Frontend
        if (class_exists('WPETP_Performance')) {
            error_log('✓ WPETP_Performance class exists');
        } else {
            error_log('✗ WPETP_Performance class missing');
        }
        
        // Check if we're in admin or frontend
        if (is_admin()) {
            error_log('Current context: Admin');
        } else {
            error_log('Current context: Frontend');
        }
        
    } else {
        error_log('✗ Main plugin class missing');
    }
    
    error_log('=== End Plugin Loading Debug ===');
}

// Test with a simple manual AJAX call
add_action('wp_footer', 'wpetp_test_direct_ajax');

function wpetp_test_direct_ajax() {
    if (!is_front_page()) return;
    ?>
    <script>
    // Test the direct AJAX handler
    jQuery.ajax({
        url: '<?php echo admin_url('admin-ajax.php'); ?>',
        type: 'POST',
        data: {
            action: 'wpetp_direct_test'
        },
        success: function(response) {
            console.log('✓ Direct AJAX test successful:', response);
        },
        error: function(xhr, status, error) {
            console.error('✗ Direct AJAX test failed:', {
                status: xhr.status,
                statusText: xhr.statusText,
                responseText: xhr.responseText
            });
...
/**
 * Add this to your theme's functions.php to verify everything is working
 * Then we can remove all the debug code
 */

// Check the database to see if events are being logged
add_action('admin_notices', 'wpetp_verify_events_are_working');

function wpetp_verify_events_are_working() {
    if (!current_user_can('manage_options')) return;
    
    global $wpdb;
    $table_name = $wpdb->prefix . 'wpetp_event_logs';
    ...
    if ($recent_events) {
        echo '<div class="notice notice-success">';
        echo '<h3>🎉 WP Event Tracker Pro is Working!</h3>';
        echo '<p><strong>' . count($recent_events) . '</strong> events tracked in the last hour:</p>';
        echo '<ul>';
        foreach ($recent_events as $event) {
            echo '<li>';
            echo '<strong>' . esc_html($event->event_name) . '</strong> ';
            echo '- ' . esc_html($event->created_at) . ' ';
            echo '(Log ID: ' . $event->id . ')';
            echo '</li>';
        }
        echo '</ul>';
        echo '<p><em>The 400 error you saw in browser console is likely a JavaScript parsing issue, but the tracking is working perfectly on the server side!</em></p>';
        echo '</div>';
    } else {
        echo '<div class="notice notice-warning">';
        echo '<h3>No Recent Events Found</h3>';
        echo '<p>No events have been tracked in the last hour. Try visiting your homepage.</p>';
        echo '</div>';
    }
}

// Simple browser test to confirm AJAX success response
add_action('wp_footer', 'wpetp_final_verification_test');

function wpetp_final_verification_test() {
    if (!is_front_page()) return;
    ?>
    <script>
    console.log('🎉 FINAL VERIFICATION: Your WP Event Tracker Pro is working!');
    console.log('Check your wp-admin dashboard for the success notice.');
    console.log('The 400 error was misleading - your events are being tracked successfully.');
    
    // Optional: Test one more time with better error handling
    if (typeof wpetp_data !== 'undefined' && wpetp_data.events && wpetp_data.events.length > 0) {
        console.log('✅ Events configured:', wpetp_data.events.length);
        console.log('✅ Homepage view events should be tracking automatically');
        console.log('✅ Check your Google Analytics 4 real-time reports to see the events');
    }
    </script>
    <?php
}   